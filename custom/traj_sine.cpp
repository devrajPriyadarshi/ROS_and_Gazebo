...
int main(int argc, char** argv) {
	...
	while(ros::ok()){
		...
		double t_max = 3*v_max/a_max;
		double omega = 4*3.14159/t_max;

		if( t < t_max){
			pos(2) = 0.25*(1 - std::cos(omega*t) );
			vel(2) = 0.25*omega*std::sin(omega*t);
			acc(2) = 0.25*omega*omega*std::cos(omega*t);
		}
		else{
			pos(2) = 0.25*(1 - std::cos(omega*t_max) );
			vel(2) = 0;
			acc(2) = 0;
		}
